...
import { makeCreditCardNumber, getArrayOfShippingOptions } from '../global/functions'
...
export default class Checkout extends React.Component {
  ...
  checkBillingInfo() {
    ...
    if (error) {
      this.displayError(
          ...
          '\nCheck the following fields: ' + fieldsError.join(', ')
      );
      return false;
    } else {
  ...
  validateCreditCardMM(nameField){
    try {
      let value = this.getValueByNameOfMember(nameField);
      if (this.getValueByNameOfMember(Fields.cc_exp_yr) > Moment().year()){
        if (value < 1) { value = '1'}
      } else if (this.getValueByNameOfMember(Fields.cc_exp_yr) == Moment().year()) {
        let month = Moment().month() + 1;
        if (value < month) { value = month.toString()}
      }
      if (value > 12) { value = '12' }
      let error = validateCreditCardMM(value);
      this.setValueForFields(nameField, value < 10 && value.length < 2? '0' + value : value, error);
      return error;
    } catch (error) {
      console.log(error)
    }
