...
function *watchFindMemberByEmail() {
   while (true) {
      const {email} = yield take(types.REQUEST_MEMBER_BY_EMAIL)
      const member  = yield call(api.findMemberByEmail, email)
      yield put(memberActions.receiveMember(member.data))
   }
}
...
// function* watchFindMemberByEmail() {
//    yield* takeEvery(types.REQUEST_MEMBER, findMemberByEmail)
// }
